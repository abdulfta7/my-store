...
export default function NewProductPage() {
  ...
  // Exactly 4 images requested by user
  ...
  useEffect(() => {
    ...
    // In a real app, you would have a GET endpoint for this or pass it from a server component
    const fetchCategories = async () => {
      try {
        // Mock fetch or actual fetch if you create a GET endpoint
        // For now we will rely on Prisma if we make a GET route, or we can just fetch all categories.
        // Let's create a temporary fetch from a generic endpoint or we assume the admin creates categories first.
        // Wait, we need a GET route for categories! Let's quickly add it.
        const res = await fetch("/api/categories"); // Assume this exists from storefront, if not we'll create it
        if (res.ok) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === "name") {
      ...
      setFormData(prev => ({ 
        ...prev, 
        slug: generatedSlug,
        sku: `PRD-${generatedSlug.substring(0, 5).toUpperCase()}-${Math.floor(Math.random() * 1000)}` 
      }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/products", {
        ...
        body: JSON.stringify({
          ...formData,
          imageUrls, // Send the array of uploaded image URLs
          specs: specs.filter(s => s.name.trim() !== "" && s.value.trim() !== ""), // Only send non-empty specs
        }),
      });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
            <div>
              ...
              <input type="text" name="name" value={formData.name} onChange={handleChange} required className="form-input" placeholder="e.g. MacBook Pro 16" />
            </div>
            <div>
        ...
        {/* Pricing */}
        <div style={{ backgroundColor: 'var(--card-bg)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          <h2 style={{ fontSize: '1.25rem', fontWeight: '700', marginBottom: '1.5rem' }}>Pricing</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', marginBottom: '0.5rem' }}>Old Price (EGP)</label>
              <input type="number" step="0.01" name="oldPrice" value={formData.oldPrice} onChange={handleChange} className="form-input" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', marginBottom: '0.5rem' }}>Discount Amount (EGP)</label>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          {specs.length === 0 ? (
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', fontStyle: 'italic' }}>No specifications added. Click the button above to add one.</p>
          ) : (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
          ...
          <button type="submit" disabled={loading} className="btn-primary" style={{ minWidth: '150px' }}>
            {loading ? <Loader2 size={20} className="animate-spin" /> : <><Save size={20} /> Save Product</>}
          </button>
        </div>
      </form>
    </div>
