...
export function CategoryEditForm({ category }: CategoryEditFormProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(category.imageUrl);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // Auto-generate slug from name only if slug was empty or user typing name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        // If preview is removed, clear the image
        imageUrl = null;
      }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <Link href="/admin/categories" style={{ padding: '0.5rem', backgroundColor: 'white', borderRadius: '8px', border: '1px solid var(--border)', color: 'var(--foreground)' }}>
            <ArrowLeft size={20} />
          </Link>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Edit Category: {category.name}</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', borderTop: '1px solid var(--border)', paddingTop: '1.5rem' }}>
          ...
          <button type="submit" disabled={loading} className="btn-primary" style={{ minWidth: '150px' }}>
            {loading ? <Loader2 size={20} className="animate-spin" /> : <><Save size={20} /> Update Category</>}
          </button>
        </div>
      </form>
    </div>
